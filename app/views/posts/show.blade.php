...
<div class="row">    
	<div class="col-sm-7">
        ...
        <p class='small text-muted'>Posted by: {{{ $post->user->fname}}} {{{ $post->user->lname }}}</p>    
        <p> {{ $post->body }} </p>
    </div>
    @if (!is_null($post->image_path))  
    <div class='col-sm-4' style='margin-top:130px; margin-left:40px;'>
        <a href='#' id='pphoto'><img src='{{{ $post->image_path }}}' class='img-responsive thumbnail'></a>
    </div>
    @endif
</div>
<div class='row'>
...
<!-- This is the disqus comment system -->
<div id="disqus_thread" style='width:550px; margin-left:15px;'></div>
...
<script>
    $('#pphoto').on('click', function (e){
        e.preventDefault();
        bootbox.dialog({
          message: "<img src='{{{ $post->image_path }}}' class='img-responsive'>",
        });
    });   
</script>

<!-- This script builds my comment system -->
<script type="text/javascript">
    /* * * CONFIGURATION VARIABLES: EDIT BEFORE PASTING INTO YOUR WEBPAGE * * */
    var disqus_shortname = 'cckblogproj'; // required: replace example with your forum shortname
